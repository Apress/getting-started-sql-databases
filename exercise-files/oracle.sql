/**	Oracle Quirks
	================================================
	Mixed SELECT:
		SELECT id, customers.* FROM customers;

	Check on names with apostrophes

	Don’t use cast(… as date) to truncate
	Use trunc(…)

	Transaction
	
	UNION & ORDER BY
		Oracle needs the alias repeated

	Fix IDENTITY GENERATED BY DEFAULT value
...
/*	3.25: String Functions
	...
	--	Also in SELECT
		SELECT customers.*, length(familyname)
		FROM customers
		WHERE length(familyname)<5;

	--	Cannot use alias in WHERE clause
		SELECT customers.*, length(familyname) AS size
		FROM customers
		WHERE size<5;		--	ERROR
...
/*	3.31: Compact ISO 8601
	...
	SELECT *
	FROM customers
	WHERE dob='19891109';			--	Doesn’t work
...
/*	3.32: Date Variations
	...
	SELECT *
	FROM customers
	WHERE dob='9 Nov 1989';			--	This works

	SELECT *
	FROM customers
	WHERE dob='November 9, 1989';	--	Doesn’t work
...
/*	3.34: BETWEEN
	...
	--	Born in the 1980s
		SELECT *
		FROM customers
		WHERE dob BETWEEN date'1980-01-01' AND date'1989-12-31';

	--	Born in the 1980s
		SELECT *
		FROM customers
		WHERE dob NOT BETWEEN date'1980-01-01' AND date'1989-12-31';
...
/*	3.36: Rewritten BETWEEN
	...
	--	Born in the 1980s
		SELECT *
		FROM customers
		WHERE dob>=date'1980-01-01' AND dob<=date'1989-12-31';
...
/*	3.40: AND vs OR
	...
	--	ALL must be true:
		SELECT *
		FROM customers
		WHERE state='QLD' AND dob<date'1980-01-01';

	--	ANY (or ALL) must be true:
		SELECT *
		FROM customers
		WHERE state='QLD' OR dob<date'1980-01-01';

/*	3.41: AND and OR
	================================================ */
	SELECT *
	FROM customers
	WHERE state='QLD' OR state='VIC' AND dob<date'1980-01-01';
...
/*	3.56: Wildcards with Dates
	================================================
	Uses Oracle Format starting with day
	================================================ */
...
/*	3.57: Regular Expressions
	...
	SELECT *
	FROM customers
	WHERE REGEXP_LIKE(lower(familyname),'[a-k][^hy].*');
...
/*	4.20: Another Calculated Column
	...
	SELECT customers.*, length(familyname) AS ln
	FROM customers
	ORDER BY ln;
...
/*	5.01: Testing Calculations
	...
	SELECT 2+5 AS result FROM dual;
...
/*	5.03: Emulating Variables
	...
	WITH vars AS (SELECT 0.1 AS taxrate FROM dual)
	SELECT
...
/*	5.15: Calculating with Numbers
	...
	SELECT 1+2*3 FROM dual;						--  1+6 = 7		NOT 9
	SELECT 12/2*3 FROM dual;					--  6*3 = 18		NOT 2
	SELECT (1+2)*3 FROM dual;					--  3*3 = 9		NOT 7
	SELECT 12 / (2*3) FROM dual;				--	12/6 = 2		NOT 18
	SELECT 2 * (3 + 4) + 3 * (4 + 5) FROM dual;	--  2*7+3*9 = 14+27 = 41
...
/*	5.16: Integer Division
	...
	SELECT 200/7 AS plain, 200.0/7 as decimalised FROM dual;

	SELECT cast(200 as float)/cast(7 as float) FROM dual;
	SELECT cast(6.5 as int) FROM dual;
...
/*	5.19: Mathematical Functions
	...
	SELECT
		acos(-1) AS pi,
		sin(45*acos(-1)/180) AS sin45,	--	Trig uses Radians
		sqrt(2) AS root2,				--	√2
		log(10,3) AS log3,
...
/*	5.25: Extracting Parts of a Date
	...
	WITH moonshot AS (
		SELECT timestamp '1969-07-20 20:17:40' AS datetime FROM dual
	)
	SELECT
		datetime,
...
/*	5.30: Search for Substring
	...
	SELECT instr('abcdefghijklmnop','m') FROM dual;
...
/*	5.32: Change Case
	...
	SELECT lower('mIxEd cAsE'), upper('mIxEd cAsE') FROM dual;

/*	5.33: Trim
	================================================ */

	SELECT rtrim(ltrim(' abcdefghijklmnop ')) FROM dual;
	SELECT trim(' abcdefghijklmnop ') FROM dual;
...
/*	5.35: Left & Right
	...
	--	left()
		SELECT substr('abcdefghijklmnop',1,5) FROM dual;
	
	--	right()
		SELECT substr('abcdefghijklmnop',-4) FROM dual;
...
/*	5.37: Sub Queries
	...
	SELECT (SELECT title FROM paintings WHERE id=123) FROM dual;
...
/*	5.43: Cast with Coalesce
	...
	SELECT
		cast(id AS varchar(5))
			||': '||givenname||' '||familyname
			||coalesce(' - '||cast(dob as varchar(12)),'')
		AS info
	FROM customers;
...
/*	5.44: Cast to Date
	...
	--	This works
		SELECT cast('20 Jul 1969' as date) AS moon_landing FROM dual;

	--	This doesn’t
		SELECT cast('tomorrow' as date) AS birthday FROM dual;
...
/*	6.08: INNER JOIN
	...
	SELECT *
	FROM minipaintings p INNER JOIN miniartists a ON p.artistid=a.id;
...
/*	6.09: OUTER JOINs
	...
	--	minipaintings on left
		SELECT *
		FROM minipaintings p LEFT OUTER JOIN miniartists a ON p.artistid=a.id;

	--	minipaintings on right
		SELECT *
		FROM miniartists a RIGHT OUTER JOIN minipaintings p ON a.id=p.artistid;

	--	without OUTER
		SELECT *
		FROM minipaintings p LEFT JOIN miniartists a ON p.artistid=a.id;

		SELECT *
		FROM miniartists a RIGHT JOIN minipaintings p ON a.id=p.artistid;

/*	6.10: Parent OUTER JOIN
	================================================ */

	SELECT *
	FROM minipaintings p RIGHT JOIN miniartists a ON p.artistid=a.id;

	SELECT *
	FROM miniartists a LEFT JOIN minipaintings p ON a.id=p.artistid;
...
/*	6.21: One to One JOIN
	...
	--	Select Columns
		...
		FROM artists a JOIN artistsdates ad ON a.id=ad.id;
...
/*	6.23: JOIN on Birthday
	...
	FROM
		artists a
		JOIN artistsdates ad ON a.id=ad.id
		JOIN customers c
			ON to_char(ad.borndate,'MM-DD')=to_char(c.dob,'MM-DD');
...
/*	6.26: Supervisor as JOIN
	...
	--	This won’t work either:
		SELECT
			id, supervisorid, givenname, familyname,
			givenname||' '||familyname as supervisor
		FROM employees e JOIN employees AS s
			ON e.supervisorid=s.id
		ORDER BY id;
	...
	--	This Works
		SELECT
			e.id, e.supervisorid, e.givenname, e.familyname,
			s.givenname||' '||s.familyname as supervisor
		FROM employees e JOIN employees AS s ON e.supervisorid=s.id
		ORDER BY e.id;
	...
	--	Include Top Level
		SELECT
			e.id, e.supervisorid, e.givenname, e.familyname,
			s.givenname||' '||s.familyname as supervisor
		FROM employees e LEFT JOIN employees s
			ON e.supervisorid=s.id
		ORDER BY e.id;
...
/*	7.28: Preparing Data for Aggregating
	...
	SELECT trunc(ordered) as ordered, total FROM sales;
...
/*	7.34: Duplicate Dates of Birth
	...
	SELECT *
	FROM customers c JOIN cte ON c.dob=cte.dob
	ORDER BY c.dob;
...
/*	7.35: Duplicate Names
	...
	SELECT *
	FROM
		customers c JOIN cte ON c.givenname=cte.givenname AND c.familyname=cte.familyname
	ORDER BY c.familyname, c.givenname;
...
/*	9.14: Customers without Sales
	...
	--	Alternative with more detail:
		...
		FROM customers c LEFT JOIN sales s ON s.customerid=c.id
		WHERE s.id IS NULL;
...
/*	9.15: Artists without Paintings
	...
	--	Using OUTER JOIN
		...
		FROM paintings p RIGHT JOIN artists a ON p.artistid=a.id
		WHERE p.id IS NULL;
...
/*	9.18: Virtual Tables
	================================================ */
		...
		SELECT 'one' AS test, cast('29 Jan 2020' as date) AS testdate FROM dual
		UNION
		SELECT 'two', cast('28 Feb 2020' as date) FROM dual
		UNION
		SELECT 'three', cast('30 Mar 2020' as date) FROM dual
		;
...
/*	9.22: State & Grand Totals
	...
	UNION
	SELECT
		0 AS statelevel, 1 as townlevel,
		state, 'total' AS town, count(*) AS count
	FROM customers
	GROUP BY state
	UNION
	SELECT
		1 AS statelevel, 1 as townlevel,
		'national', 'total' AS town, count(*)
	FROM customers
	--	Sort Results:
