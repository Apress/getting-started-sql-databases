...
/*	3.35: Date Calculation
	...
	SELECT * FROM customers WHERE dob<date('now','-40 year');
...
/*	3.56: Wildcards with Dates
	================================================
	Remember SQLite Dates are really another type
	================================================ */
...
/*	4.19: Sorting on Alias
	...
	SELECT id, givenname, familyname, died-born AS age
	FROM artists
	WHERE died-born<50	--	WHERE age<50
	ORDER BY age;
...
/*	4.22: Paged Results
	...
	--	Page 4 (skip 3 pages)
		SELECT * FROM paintings
		ORDER BY title LIMIT 20 OFFSET 3*20;
		SELECT * FROM paintings
...
/*	5.20: Approximation Functions
	...
	SELECT
		...
		round(200/7.0 + 0.5) AS ceiling,
		round(200/7.0 - 0.5) AS floor,
...
/*	5.22: Current Date & Time
	...
	SELECT current_timestamp;	--	aka	date('now'), datetime('now')
...
/*	5.24: Age Calculations
	...
	SELECT
		...
		cast(
			strftime('%Y.%m%d', 'now')- strftime('%Y.%m%d', dob)
		as int) AS age
	FROM customers;
...
/*	5.36: Format Phone Number
	...
	SELECT
		id, givenname, familyname,
		substr(phone,1,2)||' '||substr(phone,3,4)||' '||substr(phone,-4) AS phone
	FROM customers;
...
/*	5.42: Cast to String
	...
	--	Using Cast
		SELECT
			cast(id AS varchar(5))||': '||givenname||' '||familyname||' - '||cast(dob as varchar(12))
			AS info
		FROM customers;
...
/*	9.15: Artists without Paintings
	...
	--	Using OUTER JOIN
		...
		FROM artists AS a LEFT JOIN paintings AS p ON p.artistid=a.id
		WHERE p.id IS NULL;
...
/*	9.19: Using Virtual Table
	...
	WITH samples AS (
		SELECT 'one' AS test, '2020-01-29' AS testdate		UNION		SELECT 'two', '2020-02-28'		UNION		SELECT 'three', '2020-03-30'	)
	SELECT test, testdate, strftime('%Y-%m-%d',testdate,'+30 day')
	FROM samples;
